-- ============================================================
-- FIXTURE-ONLY: stub Supabase's `auth` schema.
-- Real Supabase provides `auth.users` and `auth.uid()`/`auth.jwt()` reading
-- the request JWT. Here `auth.uid()`/`auth.jwt()` read session-local GUCs
-- that fakeSupabase.ts's `signInAs()` sets, so RLS/`get_my_household_id()`
-- behave the same as in production per-request.
-- ============================================================
...
create or replace function auth.jwt() returns jsonb
  language sql stable
  as $$ select nullif(current_setting('app.current_user_jwt', true), '')::jsonb $$;
...
-- ============================================================
-- HELPER FUNCTION
-- FIXTURE NOTE: schema.md lists this before `households`/`household_memberships`
-- as documentation, but `language sql` function bodies (unlike `plpgsql`) are
-- validated against the catalog at CREATE FUNCTION time, so here it must
-- come after the table it queries.
-- ============================================================
...
-- ============================================================
-- FIXTURE NOTE: production also adds meals/weekly_plans/grocery_presets/
-- grocery_items to the `supabase_realtime` publication (see schema.md).
-- Skipped here — `realtime.ts` isn't exercised by the sync-engine/RPC
-- tests this fixture backs, and PGlite has no realtime replication.
-- ============================================================
...
-- as would otherwise bypass RLS entirely). fakeSupabase.ts runs every
-- `rpc()`/`from()` call as this role, matching how the real Supabase
-- client always talks to Postgres as `authenticated`, never as an owner.
-- ============================================================
