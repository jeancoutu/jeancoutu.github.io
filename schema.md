...
-- each affected meal's sync metadata so other devices pull the change.
-- Deliberately outside the per-aggregate sync-queue model: online-only,
-- server-authoritative, atomic, single-field — no p_base_version check.
-- A concurrent unsynced meal edit on another device conflicts normally
-- on its next upsert_meal and goes through the existing server-wins path.
-- MIGRATION: brand-new function name — plain `create or replace`, no
-- overload drop needed. No new table, RLS, index, or realtime change.
-- ============================================================
